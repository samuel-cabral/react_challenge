...
export function CartProvider({ children }: CartProviderProps): JSX.Element {
  const [cart, setCart] = useState<Product[]>(() => {
    const storagedCart = localStorage.getItem('@SupermarketShow:cart')
    ...
    try {
      ...
      if(!productAlreadyInCart) {
        ...
        if(stock.amount > 0) {
          ...
          localStorage.setItem('@SupermarketShow:cart', JSON.stringify([...cart, {...product, amount: 1}]))
          toast('Adicionado')
          return;
        }
      }
      ...
      if(productAlreadyInCart) {
        ...
        if (stock.amount > productAlreadyInCart.amount) {
          ...
          setCart(updatedCart)
          localStorage.setItem('@SupermarketShow:cart', JSON.stringify(updatedCart))
          return;
        } else {
